...
class Game:
    def __init__(self, nb_team:int, challenge_id:int = randint(0,1)) -> None:
        ...
        self.teams = [Teams(i, next(t_name, None), self.nb_users, self.__challenge_id) for i in range(nb_team + 1)]
        self.users = []
    ...
    def change_user_team(self, id_user:int, id_team:int):
        ...
        self.users[id_user].part = self.teams[id_team].get_nb_player() % self.nb_users
